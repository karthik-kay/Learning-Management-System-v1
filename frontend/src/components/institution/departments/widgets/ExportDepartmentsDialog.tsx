...
interface ExportDepartmentsDialogProps {
  open: boolean;
  filters: Record<string, unknown>;
  onOpenChange: (open: boolean) => void;
}

export function ExportDepartmentsDialog({
  open,
  filters,
  onOpenChange,
}: ExportDepartmentsDialogProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    createExport.mutate(
      { report_type: "departments", export_format: format, filters },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Create an async export job for the current department view.
          </DialogDescription>
        </DialogHeader>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">
            ...
            <p className="mt-1 break-words">
              {Object.keys(filters).length ? JSON.stringify(filters) : "All departments"}
            </p>
          </div>
