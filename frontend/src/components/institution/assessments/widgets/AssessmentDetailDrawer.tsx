"use client";

import { ClipboardCheck, FileText, UsersRound } from "lucide-react";

import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
} from "@/components/ui/sheet";
import type {
  Assignment,
  EvaluationComponent,
  StudentComponentScore,
} from "@/types/institution";

interface AssessmentDetailDrawerProps {
  open: boolean;
  component?: EvaluationComponent | null;
  assignments: Assignment[];
  scores: StudentComponentScore[];
  onOpenChange: (open: boolean) => void;
}

export function AssessmentDetailDrawer({
  open,
  component,
  assignments,
  scores,
  onOpenChange,
}: AssessmentDetailDrawerProps) {
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader className="border-b border-slate-200 px-6 py-5">
          <SheetTitle>Assessment preview</SheetTitle>
          <SheetDescription>Component rules, assignments, and scores.</SheetDescription>
        </SheetHeader>

        {component ? (
          <div className="space-y-6 p-6">
            <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
              <div className="flex items-start gap-4">
                <div className="flex size-14 items-center justify-center rounded-2xl bg-[#E7F6F5] text-[#22577A]">
                  <ClipboardCheck className="size-6" />
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="text-xl font-semibold text-[#0F172A]">
                    {component.name}
                  </h3>
                  <p className="mt-1 text-sm text-slate-500">
                    {component.subject_name || "N/A"} · {component.batch_name || "N/A"}
                  </p>
                </div>
              </div>
            </section>

            <section className="grid gap-3 sm:grid-cols-3">
              <Metric label="Max marks" value={component.max_marks} />
              <Metric label="Weightage" value={`${component.weightage}%`} />
              <Metric label="Scores" value={scores.length} />
            </section>

            <AssociationSection
              title="Assignments"
              icon={<FileText className="size-4" />}
              emptyText="No assignments connected to this component."
            >
              {assignments.map((assignment) => (
                <AssociationRow
                  key={assignment.id}
                  title={assignment.title}
                  meta={assignment.subject_name}
                  value={assignment.due_date}
                />
              ))}
            </AssociationSection>

            <AssociationSection
              title="Recent scores"
              icon={<UsersRound className="size-4" />}
              emptyText="No scores recorded yet."
            >
              {scores.slice(0, 8).map((score) => (
                <AssociationRow
                  key={score.id}
                  title={score.student_name || "N/A"}
                  meta={score.enrollment_number || "N/A"}
                  value={score.is_absent ? "Absent" : String(score.marks_obtained)}
                />
              ))}
            </AssociationSection>
          </div>
        ) : (
          <p className="p-6 text-sm text-slate-500">Select an assessment.</p>
        )}
      </SheetContent>
    </Sheet>
  );
}

function Metric({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <p className="text-sm text-slate-500">{label}</p>
      <p className="mt-2 text-xl font-semibold text-[#0F172A]">{value}</p>
    </div>
  );
}

function AssociationSection({
  title,
  icon,
  emptyText,
  children,
}: {
  title: string;
  icon: React.ReactNode;
  emptyText: string;
  children: React.ReactNode[];
}) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
      <div className="mb-4 flex items-center gap-2">
        <span className="text-[#38A3A5]">{icon}</span>
        <h4 className="font-semibold text-[#0F172A]">{title}</h4>
      </div>
      <div className="space-y-3">
        {children.length ? (
          children
        ) : (
          <p className="rounded-xl border border-dashed border-slate-200 bg-white p-4 text-sm text-slate-500">
            {emptyText}
          </p>
        )}
      </div>
    </section>
  );
}

function AssociationRow({
  title,
  meta,
  value,
}: {
  title: string;
  meta: string;
  value: string;
}) {
  return (
    <div className="flex items-start justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4">
      <div>
        <p className="font-semibold text-[#0F172A]">{title}</p>
        <p className="mt-1 text-xs text-slate-500">{meta}</p>
      </div>
      <span className="rounded-full bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600">
        {value}
      </span>
    </div>
  );
}
